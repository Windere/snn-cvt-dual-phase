...
def cvtFn(iso_snn: IsomorphicSNN, layer_idx):
    ...
    num_layer = SpikeBasicBlock.num_layer
    ...
    for i in range(len(kwargs['thresh'])):
        if iso_snn.enable_shift:
            shift.append(kwargs['thresh'][i] / (2 * iso_snn.nb_steps))
        else:
            shift.append(0)
        init_mem.append(kwargs['thresh'][i] / 2 if iso_snn.enable_init_volt else 0)
        # pass
