...
from util.fold_bn import StraightThrough, ScaleStraightThrough
...
def balanceWeight(net, layer_names, factors, idx=0, prev=None, mode='layer_wise', prefix=''):
    # todo: add supports for channel-wise and  neuron-wise conversion
    if mode != 'layer_wise':
        raise NotImplementedError('neuron_wise and channel_wise are not supported now')
    for name, module in net.named_children():
        full_name = name if not prefix else prefix + '.' + name
        if name == 'shortcut':
            if isinstance(module, nn.Sequential):
                assert idx > 0, "The network topology does not follow the protocol"
                module[0].weight.data *= factors[idx - net.num_layer] / factors[idx]  # for basic block
                if module[0].bias is not None:
                    module[0].bias.data /= factors[idx]
            elif isinstance(module, ScaleStraightThrough):
                module.scale = factors[idx - net.num_layer] / factors[idx]  # for bottle-neck block
            print(layer_names[idx], layer_names[idx - net.num_layer])
            continue
        if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
            prev = module
        elif idx < len(layer_names) and full_name == layer_names[idx]:
            if prev is not None:
                tfactor = factors[idx - 1] if idx > 0 else 1.0
                prev.weight.data *= tfactor / factors[idx]
                if prev.bias is not None:
            ...
            prev, idx = balanceWeight(module, layer_names, factors, idx, prev, mode=mode, prefix=full_name)
    return prev, idx
