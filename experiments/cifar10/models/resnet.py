...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, act_fun=nn.ReLU()):
...
class SpikeBasicBlock(nn.Module):
    ...
    def __init__(self, block, thresh, decay, init_mem, shift, cell=LIFCell, **kwargs_lif):
        super(SpikeBasicBlock, self).__init__()
        self.conv1 = cell(block.conv1, thresh=thresh[0], decay=decay[0], shift=shift[0],
                          init_mem=init_mem[0], **kwargs_lif)
        self.conv2 = cell(block.conv2, thresh=thresh[1], decay=decay[1], shift=shift[1],
...
def ResNetX(depth, act_fun, num_class=10, modified=False):
