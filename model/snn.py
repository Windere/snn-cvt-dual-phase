...
import torch
import numpy as np
from .function import td_Dropout, tdBatchNorm, mul
import torch.nn.functional as F
import torch.nn as nn
from model.surrogate_act import RectSurrogate
...
class AIFCell(torch.nn.Module):
    #  the status updating as a class
    """"
    the simplest iterative equation of LIF equations
    """

    def __init__(self, func, spike_fn, decay=None, thresh=None, mode='psp', init_mem=0, shift=0, negative_slope=0.01):
        super(AIFCell, self).__init__()
        self.func = func
        self.decay = decay
        self.thresh = thresh
        self.negative_slope = negative_slope
        self.neg_thresh = 1 / negative_slope * thresh
        self.spike_fn = spike_fn
        self.mode = mode
        self.mem = 0
        self.init_mem = init_mem
    ...
    def forward(self, x, extern_current=0):
        ...
        :return:
        """
        if self.t == 0 and isinstance(self.mem, torch.Tensor):
            self.mem = self.mem.to(x.device)
        self.t += 1
        current = self.func(x)  # input current #todo: check here
        self.mem = self.decay * self.mem + (current + extern_current) * self.thresh / abs(self.thresh) + self.shift
        pos_spike = self.spike_fn(self.mem - abs(self.thresh))
        neg_spike = -self.spike_fn(-self.mem - abs(self.neg_thresh))
        self.mem = self.mem - pos_spike * abs(self.thresh) - neg_spike * abs(self.neg_thresh)
    ...
    def reset_parameters(self):
        if (self.thresh is None):
            self.thresh = torch.nn.Parameter(torch.empty(1), requires_grad=True)
            torch.nn.init.normal_(self.thresh, mean=1.0, std=0.01)
            self.neg_thresh = 1 / self.negative_slope * self.thresh
    ...
    def clamp(self):
        self.decay.data.clamp_(0., 1.)
        self.thresh.data.clamp_(min=0.)
        self.neg_thresh.data.clamp_(min=0.)
...
class LIFCell(torch.nn.Module):
    ...
    def forward(self, x, extern_current=0):
        ...
        current = self.func(x)  # input current #todo: check here
        self.mem = self.decay * self.mem + (current + extern_current) * self.thresh / abs(self.thresh) + self.shift
        spike = self.spike_fn(self.mem - abs(self.thresh))
        self.mem = self.mem - spike * abs(self.thresh)
        if self.mode == 'psp':
            spike *= self.thresh
        return spike
...
class IFAct(torch.nn.Module):
    #  the status updating as a class
    """"
    the simplest iterative equation of LIF equations
    """

    def __init__(self, spike_fn=RectSurrogate.apply, thresh=None, mode='psp', init_mem=0, shift=0):
        super(IFAct, self).__init__()
        self.thresh = thresh
        self.spike_fn = spike_fn
        self.mode = mode
        self.mem = 0
    ...
    def forward(self, x):
        ...
        if self.t == 0 and isinstance(self.mem, torch.Tensor):
            self.mem = self.mem.to(x.device)
        self.t += 1
        current = x + self.shift  # input current
