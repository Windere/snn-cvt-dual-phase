...
"""
    The code modified from https://github.com/yhhhli/SNN_Calibration
"""
...
class ScaleStraightThrough(nn.Module):
    def __init__(self, channel_num: int = 1, scale=1):
        self.scale = scale
        super().__init__()
...
def search_fold_and_remove_bn(model, replace=True):
    model.eval()
    prev = None
    for n, m in model.named_children():
        if is_bn(m) and is_absorbing(prev):
            ...
            prev = search_fold_and_remove_bn(m, replace)
    return prev
